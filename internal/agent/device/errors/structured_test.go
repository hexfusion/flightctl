...
func TestFormatError(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			err: fmt.Errorf(`applications: parsing apps: ensuring app type: required label not found: %w: declared "compose" discovered "quadlet": %w`,
				ErrAppTypeMismatch,
		...
		},
		{
			...
			err: fmt.Errorf("os: stage image: bootc switch failed: image not found: %w",
		...
		},
		{
			...
			err: fmt.Errorf("failed to update status with decommission started Condition: failed to update decommission started status: failed to push status: %w",
		...
		},
		{
			...
			err: fmt.Errorf("systemd: %w: invalid regex: [bad, error: unexpected end of pattern: %w",
				ErrInvalidPatterns,
		...
		},
		{
			...
			err: fmt.Errorf("hooks: failed to execute BeforeUpdating hook action #1: command failed (%w 1): %w",
				ErrExitCode,
		...
		},
		{
			...
			err: fmt.Errorf("update policy: %w: strconv.ParseInt: parsing: syntax error: %w",
				ErrInt64Conversion,
		...
		},
		{
			...
			err: fmt.Errorf(`failed to apply configuration: write file /etc/app.conf: failed to create directory "/etc/app": permission denied: %w`,
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentConfig, ErrPermissionDenied)),
		...
		},
		{
			...
			err: fmt.Errorf("installing application: writing env file: no space left on device: %w",
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentApplications, ErrNoSpaceLeft)),
		...
		},
		{
			...
			err: fmt.Errorf("resources: context canceled: %w",
				errors.Join(ErrPhasePreparing, ErrComponentResources, context.Canceled)),
		...
		},
		{
			...
			err: fmt.Errorf(`reading hook actions from "/etc/flightctl/hooks.d/pre-update.yaml": file does not exist: %w`,
				errors.Join(ErrPhasePreparing, ErrComponentHooks, ErrDoesNotExist)),
		...
		},
		{
			...
			err: fmt.Errorf("failed to apply configuration: failed to decode base64 content: illegal base64: %w",
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentConfig, ErrFailedToDecode)),
		...
		},
		{
			...
			err: fmt.Errorf("prefetch: prefetch collector 0 failed: getting OS status: bootc status: command failed: %w",
				errors.Join(ErrPhasePreparing, ErrComponentPrefetch, ErrGettingBootcStatus)),
		...
		},
		{
			...
			err: fmt.Errorf("download policy: %w: version number: -1: %w",
				ErrNegativeValue,
		...
		},
		{
			...
			err: fmt.Errorf(`removing application: removing application: remove path "/var/lib/app": permission denied: %w`,
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentApplications, ErrPermissionDenied)),
		...
		},
		{
			...
			err: fmt.Errorf(`error executing actions: starting target myapp.target: service: "myapp.service" logs: failed to start: %w`,
				errors.Join(ErrPhaseActivatingConfig, ErrComponentApplications, ErrInstallingApplication)),
		...
		},
		{
			...
			err: fmt.Errorf(`error executing actions: creating volumes: pulling image volume: inspect volume "data-vol": podman failed: %w`,
				errors.Join(ErrPhaseActivatingConfig, ErrComponentApplications, ErrPodmanFailed)),
		...
		},
		{
			...
			err: fmt.Errorf("prefetch: prefetch collector 1 failed: collecting nested OCI targets: getting image digest for quay.io/myapp:v1: podman inspect failed: %w",
				errors.Join(ErrPhasePreparing, ErrComponentPrefetch, ErrGettingImageDigest)),
		...
		},
		{
			...
			err: fmt.Errorf("error executing actions: removing: stopping target myapp.target: failed to stop: %w",
				errors.Join(ErrPhaseActivatingConfig, ErrComponentApplications, ErrRemovingApplication)),
		...
		},
		{
			...
			err: fmt.Errorf(`installing application: %w: unsupported content encoding: "gzip": %w`,
				ErrDecodingApplicationContent,
		...
		},
		{
			...
			err: fmt.Errorf(`resources: time: invalid duration "30 minutes": %w`,
				errors.Join(ErrPhasePreparing, ErrComponentResources, ErrTime)),
		...
		},
		{
			...
			err: fmt.Errorf("applications: verify app provider: no retry: verifying inline: %w: podman not found: %w",
				ErrEnsuringDependencies,
				errors.Join(ErrPhasePreparing, ErrComponentApplications)),
			expectedErr: `While Preparing, applications failed: System issue - Internal error occurred`,
		},
		{
			name: "component: applications",
			err: fmt.Errorf("applications: parsing apps: %w: json error: %w",
				ErrGettingVolumes,
				errors.Join(ErrPhasePreparing, ErrComponentApplications)),
			expectedErr: `While Preparing, applications failed: System issue - Internal error occurred`,
		},
		{
			...
			err: fmt.Errorf("failed to apply configuration: safecast.ToUint32 overflow: %w",
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentConfig, ErrUnmarshalSpec)),
			expectedErr: `While ApplyingUpdate, config failed: System issue - Internal error occurred`,
		},
		{
			name: "component: systemd",
			err: fmt.Errorf("systemd: %w: regex compilation failed: %w",
				ErrInvalidPatterns,
				errors.Join(ErrPhaseApplyingUpdate, ErrComponentSystemd)),
		...
		},
		{
			...
			err: fmt.Errorf("failed to initiate system reboot: reboot systemd: failed to start: %w",
				errors.Join(ErrPhaseActivatingConfig, ErrComponentLifecycle, ErrInstallingApplication)),
			expectedErr: `While Rebooting, lifecycle failed: System issue - Internal error occurred`,
		},
		{
			name: "component: os",
			err: fmt.Errorf("%w: into a valid bootc target: invalid format: %w",
				ErrUnableToParseImageReference,
		...
		},
		{
			...
			err: fmt.Errorf("resources: AsDiskResourceMonitorSpec() error: json unmarshal: %w",
				errors.Join(ErrPhasePreparing, ErrComponentResources, ErrMarshal)),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.expectedErr, stripTimestamp(se.buildMessage()))
		})
	}
}
