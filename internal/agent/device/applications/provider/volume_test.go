  ...
  data:
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			volumeManager, err := NewVolumeManager(log, tt.appName, newTestImageApplicationVolumes(tt.volumes))
			require.NoError(err)
...
func newTestImageApplicationVolumes(names []string) *[]v1alpha1.ApplicationVolume {
	...
	for _, volName := range names {
		...
		vol.FromImageVolumeProviderSpec(spec)
