	...
	tests := []struct {
	...
	}{
		{
			...
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("auth-config", []v1alpha1.FileSpec{
					{
		...
		},
		{
			...
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("other-config", []v1alpha1.FileSpec{
					{
		...
		},
		{
			...
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("system-config", []v1alpha1.FileSpec{
					{
...
func TestAuthFromSpec(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("registry-config", []v1alpha1.FileSpec{
					{
		...
		{
			name: "provider conversion error gracefully handled",
			deviceSpec: &v1alpha1.DeviceSpec{
				Config: &[]v1alpha1.ConfigProviderSpec{
					{}, // Invalid provider - neither Inline nor ConfigMapRef set
					createInlineConfigProvider("auth-config", []v1alpha1.FileSpec{
						{
							Path:    "/etc/containers/auth.json",
			...
			authPath:      "/etc/containers/auth.json",
			expectedAuth:  `{"auths":{"valid.registry":{"auth":"validtoken"}}}`,
			expectedFound: true,
		},
		{
			name: "multiple providers - auth found in second",
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("hostname-config", []v1alpha1.FileSpec{
					{
						Path:    "/etc/hostname",
						Content: "testhost",
					},
		...
		},
		{
			...
			deviceSpec: createDeviceSpecWithInlineConfig([]v1alpha1.ConfigProviderSpec{
				createInlineConfigProvider("multi-config", []v1alpha1.FileSpec{
					{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			log := log.NewPrefixLogger("test")
			auth, found := authFromSpec(log, tt.deviceSpec, tt.authPath)
			require.Equal(tt.expectedFound, found)
			require.Equal(tt.expectedAuth, auth)
		})
	}
}

func createInlineConfigProvider(name string, files []v1alpha1.FileSpec) v1alpha1.ConfigProviderSpec {
	var provider v1alpha1.ConfigProviderSpec
	_ = provider.FromInlineConfigProviderSpec(v1alpha1.InlineConfigProviderSpec{
		Name:   name,
		Inline: files,
	})
	return provider
