...
func (t *tpmProvider) initializeTPMCredential(ctx context.Context) error {
	...
	exists, err := t.rw.PathExists(DefaultParentCredentialPath)
	if err != nil {
	...
	if exists {
		...
		if err := t.ensureParentSystemdDropin(ctx); err != nil {
			t.log.Warnf("Failed to ensure systemd drop-in: %v", err)
		}
		return nil
	}
	...
	// log the password hex for debugging (first 4 bytes only)
	if len(password) >= 4 {
		t.log.Tracef("Password starts with: %s... (%d bytes total)",
			hex.EncodeToString(password[:4]), len(password))
	}

	t.log.Info("TPM storage password credential initialized successfully")
	t.log.Infof("Sealed credential stored at: %s", DefaultParentCredentialPath)
